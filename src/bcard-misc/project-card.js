const extract_vertical_horizontal_const = () => {
  ...
  const height_const =
    projectRows[1].getBoundingClientRect().top -
    projectRows[0].getBoundingClientRect().top;
  const width_const =
    projectRowCards[1].getBoundingClientRect().left -
...
const nav_project_horiz = (x_dir) => {
  ...
  for (let i = 1; i < 4; i++) {
    ...
    let cards = Array.from(
      document.querySelectorAll(`.project-row:nth-child(${i}) .project-card`),
    );
    ...
    lastcard.addEventListener(
      ...
      () => {
        ...
        lastcard.children[0].style.cssText = '--rotateX: 0; --rotateY: 0;';
    ...
    cards[1].addEventListener(
      ...
      () => {
        ...
        copies[idx_change].addEventListener(
          'animationend',
          () => {
            copies[idx_change].classList.remove('last-card-motion2');
            copies[idx_change].children[0].classList.remove(
...
const nav_project_vert = (y_dir) => {
  ...
  firstrow.addEventListener(
    'animationend',
    () => {
      firstrow.style.cssText = '--opacity: 0; --translateX: 5vw;';
      card_rows.forEach((row, idx) => {
        ...
        Array.from(row.children).forEach((card, idx2) => {
          card.children[0].style.cssText = `
                ...
                `;
          card.children[0].classList.add('first-row-card-content-motion2');
        });
      ...
      card_rows.forEach((row, idx) => {
        ...
        Array.from(row.children).forEach((card, idx2) => {
          card.children[0].style.cssText = `
                    ...
                    --rotateY: ${-(idx2 - center_x)};
                `;
          card.children[0].classList.remove('first-row-card-content-motion2');
        });
        ...
        Array.from(copy.children).forEach((card, idx2) => {
          ...
          if (idx === idx_change) {
            card.children[0].style.cssText = `
                        ...
                        --rotateY: ${-(idx2 - center_x)};
                    `;
            card.children[0].classList.add('first-row-card-content-motion3');
          }
        });
        if (idx === idx_change) {
      ...
      copies[idx_change].addEventListener(
        ...
        () => {
          ...
          Array.from(copies[idx_change].children).forEach((card, idx) => {
            card.children[0].classList.remove('first-row-card-content-motion3');
            card.children[0].style.cssText = '';
          });
          copies[idx_change].style.cssText = '';
        },
